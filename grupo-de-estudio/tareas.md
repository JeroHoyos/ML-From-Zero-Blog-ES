# Tareas

## Mensuales

#### Mes - 1

| Tarea                                                                   | Descripción                                                                                    |
| ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Competencia                                                             | Kaggle: [Predicting Heart Disease](https://www.kaggle.com/competitions/playground-series-s6e2) |
| [An Introduction to Statistical Learning](https://www.statlearning.com) | Capítulo 2: Page 15 - 69                                                                       |

## Semanales

### Semana - 1

| Ejercicios                                                                              |
| --------------------------------------------------------------------------------------- |
| [Derivative of a Polynomial](https://www.deep-ml.com/problems/116?returnTo=paths)       |
| [Dot Product Calculator](https://www.deep-ml.com/problems/83?returnTo=paths)            |
| [Descriptive Statistics Calculator](https://www.deep-ml.com/problems/78?returnTo=paths) |

### Semana - 2

| Ejercicios                                                                                         |
| -------------------------------------------------------------------------------------------------- |
| [Product Rule for Derivatives](https://www.deep-ml.com/problems/309?returnTo=paths)                |
| [Compute the Cross Product of Two 3D Vectors](https://www.deep-ml.com/problems/118?returnTo=paths) |
| [Calculate Covariance Matrix](https://www.deep-ml.com/problems/10?returnTo=paths)                  |

### Semana - 3

| Ejercicios                                                                              |
| --------------------------------------------------------------------------------------- |
| [Quotient Rule for Derivatives](https://www.deep-ml.com/problems/312?returnTo=paths)    |
| [Gradient Direction and Magnitude](https://www.deep-ml.com/problems/308?returnTo=paths) |
| [Vector Element-wise Sum](https://www.deep-ml.com/problems/121?returnTo=paths)          |

### Semana - 4

| Ejercicios                                                                                       |
| ------------------------------------------------------------------------------------------------ |
| [Calculate P50/P95/P99 Latency Percentiles](https://www.deep-ml.com/problems/293?returnTo=paths) |
| [Matrix Transformation](https://www.deep-ml.com/problems/7)                                      |
| [Solve Linear Equations using Jacobi Method](https://www.deep-ml.com/problems/11)                |

### Semana - 5

| Ejercicios                                                                                          |
| --------------------------------------------------------------------------------------------------- |
| [Singular Value Decomposition (SVD) of 2x2 Matrix](https://www.deep-ml.com/problems/12)             |
| [Determinant of a 4x4 Matrix using Laplace's Expansion (hard)](https://www.deep-ml.com/problems/13) |
| [Linear Regression Using Normal Equation](https://www.deep-ml.com/problems/14)                      |

### Semana - 6

| Ejercicios                                                                      |
| ------------------------------------------------------------------------------- |
| [Linear Regression Using Gradient Descent](https://www.deep-ml.com/problems/15) |
| [Feature Scaling Implementation](https://www.deep-ml.com/problems/16)           |
| [K-Means Clustering](https://www.deep-ml.com/problems/17)                       |
